...
export default function SkillCoursesTab() {
  ...
  const [selectedCourse, setSelectedCourse] = useState<SkillCourse | null>(
    null,
  );
    ...
    try {
      ...
      if (raw) {
        ...
        if (lvl === "10th") qualKey = "10th";
          ...
          qualKey = stream === "commerce" ? "12th_com" : "12th_sci";
  ...
  // ── Filter Courses ───────────────────────────────────────────
  const filteredCourses: SkillCourse[] = (
    selectedCategory?.courses ?? []
  ).filter((c) => {
    if (activeFilter === "all") return true;
    if (activeFilter === "high_salary") return c.salaryMin >= 20000;
    if (activeFilter === "short_duration")
      return c.duration.toLowerCase().includes("month")
        ? parseInt(c.duration) <= 3
        : true;
    if (activeFilter === "free") return c.isFree === true;
    if (activeFilter === "remote") return c.tags.includes("remote");
    if (activeFilter === "self_employment")
      return c.tags.includes("self_employment");
        ...
        {SKILL_CATEGORIES.map((cat) => {
          ...
          return (
            ...
            >
              ...
              <View
                style={[styles.catIconCircle, { backgroundColor: cat.iconBg }]}
              >
                <Text style={styles.catEmoji}>{cat.icon}</Text>
              </View>
              <Text
                style={[styles.catTitle, { color: cat.accentColor }]}
                numberOfLines={2}
              >
                {cat.title}
              </Text>
      ...
      <View style={styles.flex1}>
        ...
        <View style={styles.backRow}>
          ...
          <View
            style={[
              styles.catIconSmall,
              { backgroundColor: selectedCategory.iconBg },
            ]}
          >
            <Text style={{ fontSize: 14 }}>{selectedCategory.icon}</Text>
          </View>
          <Text
            style={[styles.backTitle, { color: selectedCategory.accentColor }]}
            numberOfLines={1}
          >
        ...
        >
          ...
          >
            <View style={styles.detailHeroTop}>
              <View style={{ flex: 1 }}>
                <Text style={[styles.detailTitle, { color: "#1A1A2E" }]}>
                  {selectedCourse.name}
                </Text>
                <View style={styles.detailBadgesRow}>
                  {selectedCourse.badges.map((b) => (
                    <CourseBadge key={b} label={b} />
                  ))}
                </View>
              </View>
            ...
            >
              ...
              <Text style={[styles.salaryBoxValue, { color: accent }]}>
                {selectedCourse.salaryLabel}
              </Text>
            </View>
          </View>
